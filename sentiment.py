...
def stock_sentiment_analysis():
    ...
    def fetch_news_google(stock_name):
        ...
        try:
            ...
            return news_list[:20]  # Returns list (even if empty), no warning here
        except requests.exceptions.ConnectionError:
    ...
    def get_stock_news(stock_name):
        ...
        google_articles = fetch_news_google(stock_name)

        all_articles = gnews_articles + google_articles

        if not all_articles:  # Show warning only if both lists are empty
            st.warning(f"⚠️ No articles found for '{stock_name}' on Google News and GNews.")
